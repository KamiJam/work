;(function($){
    ...
    $('body').append('<style>'+
                ...
                '.sel-box .btn{background:#41B1E6;overflow:hidden;}'+
                '.sel-box .btn1{overflow:hidden;width:22px;height:22px;float:left;padding:11px 12px;box-sizing:content-box;}'+
                ...
                '.sel-con .border{height:34px;border:solid 1px #b2b2b2;border-width:1px 0;position:fixed;bottom:72px;left:0;right:0;pointer-events:none;}'+
    ...
    //取消ios在zepto下的穿透事件
    $(".sel-con").bind("touchend", function (event) {
    ...
    function dataFrame(ele){
        ...
        for(var i=0;i<ele.length;i++){
            eleText += '<div class="ele" data-value="'+ele[i].value+'">'+ele[i].text+'</div>';
        };
    ...
    $.scrEvent = function(params){
        ...
        $(evEle).bind(type, function (){
            ...
            $('.sel-con .scroll').scroll(function(){
                ...
                scText = {
                	text: $(this).find('.ele').eq(scNum).text(),
                	value: $(this).find('.ele').eq(scNum).attr("data-value")
                };
                // 停止锁定
